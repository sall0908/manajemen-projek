...
class BoardController extends Controller
{
    public function index($project_id)
    {
        ...
        return view('developer.board', compact('project', 'todo', 'inProgress', 'review', 'done'));
    }
    ...
    public function store(Request $request, $project_id)
    {
        ...
        // Pastikan developer hanya bisa menambah subtask di proyek mereka
    ...
    public function markInProgress($card_id)
    {
        ...
        // Pastikan developer hanya bisa update card yang ditugaskan padanya
    ...
    public function stopTimeLog($card_id)
    {
        ...
        // Pastikan developer hanya bisa stop time log di card yang ditugaskan padanya
        $isAssigned = \Illuminate\Support\Facades\DB::table('card_assignments')
            ->where('card_id', $card_id)
            ->where('user_id', Auth::user()->user_id)
            ->exists();
    ...
    public function getRunningTimeLog($card_id)
    {
        ...
        // Pastikan developer hanya bisa lihat time log di card yang ditugaskan padanya
        $isAssigned = \Illuminate\Support\Facades\DB::table('card_assignments')
            ->where('card_id', $card_id)
            ->where('user_id', Auth::user()->user_id)
            ->exists();
    ...
    // Update SubTask status (Developer restricted - only until review, not done)
    public function updateSubtaskStatus($sub_task_id, Request $request)
    {
        ...
        // Developer tidak boleh set status 'done'
        if ($request->status === 'done') {
            abort(403, 'Developer tidak dapat menandai SubTask sebagai Done. Hanya TeamLeader yang dapat melakukan ini.');
        }
