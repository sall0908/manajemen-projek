            ...
            <div id="cardModal{{ $card->card_id }}" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center overflow-y-auto">
                <div class="bg-white rounded-xl shadow-2xl w-full max-w-4xl mx-4 my-8 p-6">
                    ...
                    <div class="bg-blue-50 rounded-lg p-4 mb-6">
                        <h3 class="font-bold text-blue-900 mb-3">➕ Tambah SubTask Baru</h3>
                        <form action="{{ route('developer.project.cards.store', $project->project_id) }}" method="POST" class="space-y-3">
                            @csrf
                            <input type="hidden" name="card_id" value="{{ $card->card_id }}">

                            <input name="title" type="text" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Judul SubTask" required>

                            <textarea name="description" rows="2" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Deskripsi SubTask (opsional)"></textarea>
                                ...
                                <div class="space-y-2">
                                    @forelse($card->subTasks->where('status', 'todo') as $st)
                                        <div class="bg-white rounded p-3 shadow-sm">
                                            ...
                                            @endif
                                            <form action="{{ route('developer.subtasks.update', $st->sub_task_id) }}" method="POST" class="mt-2">
                                                @csrf
                                                @method('PATCH')
                                    ...
                                    @forelse($card->subTasks->where('status', 'in_progress') as $st)
                                        <div class="bg-white rounded p-3 shadow-sm">
                                            ...
                                            <div class="flex gap-1">
                                                ...
                                                @if($runningLog)
                                                    <form action="{{ route('developer.subtasks.stopTimer', $st->sub_task_id) }}" method="POST" class="flex-1">
                                                        @csrf
                                                        @method('PATCH')
                                                ...
                                                @else
                                                    <form action="{{ route('developer.subtasks.startTimer', $st->sub_task_id) }}" method="POST" class="flex-1">
                                                        @csrf
                                                        @method('PATCH')
                                                ...
                                                <form action="{{ route('developer.subtasks.update', $st->sub_task_id) }}" method="POST" class="flex-1">
                                                    @csrf
                                                    @method('PATCH')
